...
**Important:** for same-character preference chains, eligible outcomes are sorted by **descending constellation**, not preference rank - **within each scheduling class**. A chain constellation whose roadmap goal is **blocked** (§9: an unsatisfied lower-constellation goal for the same character exists) **and** whose character also has a banner strictly after the current one is a **later progression objective**: it remains in the menu but sorts **behind** the nearer objectives, because the roadmap schedules that reach for the later banner (the planner is re-run after the account update). `recommend()` promotes such a tagged outcome back to the lead only under the **later-progression eligibility rule**: pursuing it now must be an ordinary recommendation (probability ≥ `MINIMUM_OUTCOME_PROBABILITY`) at its largest cap that keeps every higher-priority protected goal safe, and it must be the deeper constellation - then one plan entry targets it and the Phase 4 simulator pulls through the nearer milestones toward it (the cumulative C0→C2 progression; §4.2, §12). Below that bar the active milestone leads. Without a strictly later banner, the deeper reach stays in front (that banner is its only remaining opportunity), and chain constellations with no matching roadmap goal are never demoted. This is intentional in the current implementation because a deeper constellation subsumes lower constellations for the same character. Do not change this casually; it is a key behavior to understand when debugging recommendation selection.
